...
trait Publishing {
  ...
  def publish(exchangeName: Expression[String], routingKey: Expression[String], body: Either[Expression[String], String], replyToProperty: Option[String] = None, headers: Map[String, AnyRef] = Map.empty): AmqpRequestBuilder = {
    val bb = new BasicProperties.Builder()
    replyToProperty.map(bb.replyTo(_))
    bb.headers(headers)
    val b = body match {
    ...
    // requestName: Expression[String], exchangeName: Expression[String], routingKey: Expression[String], body: Either[Expression[Array[Byte]], Array[Byte]]
    publish(PublishRequestAsync(this.requestName, exchangeName, routingKey, bb.build(), b))
  }
