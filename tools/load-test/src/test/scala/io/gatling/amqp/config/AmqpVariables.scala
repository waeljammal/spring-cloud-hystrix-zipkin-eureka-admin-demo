...
trait AmqpVariables { this: AmqpProtocol =>
  ...
  protected def setupVariables(system: ActorSystem, statsEngine: StatsEngine): Unit = {
    ...
    _manage = Some(system.actorOf(AmqpManage.props(statsEngine, this), "AmqpManage".concat(UUID.randomUUID().toString)))
    _nacker = Some(system.actorOf(AmqpNacker.props(statsEngine, this), "AmqpNacker".concat(UUID.randomUUID().toString)))
    _router = Some(system.actorOf(AmqpRouter.props(statsEngine, this), "AmqpRouter".concat(UUID.randomUUID().toString)))
    _tracer = Some(system.actorOf(AmqpTracer.props(statsEngine, this), "AmqpTracer".concat(UUID.randomUUID().toString)))
  }
}
